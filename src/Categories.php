...
class Categories implements \JsonSerializable
{
    ...
    public function all()
    {
        ...
        foreach( $this->arr() as $key => $item ) {
            $category = Category::create($key, $item);
     ...
     * @return Category
     */
    public function get(string $category, string $subCategory = null): ?Category
    {
        if ( ! $subCategory)  {
            return isset($this->arr()[$category]) ?
                Category::create($category, $this->arr()[$category]) : null;
        }
        return isset($this->arr()[$category]['children'][$subCategory]) ?
            Category::create($subCategory, $this->arr()[$category]['children'][$subCategory]) : null;
    }
     ...
     * @return bool
     */
    public function has(string $category, string $subCategory = null): bool
    {
        return $this->get($category, $subCategory) !== null;
    }
